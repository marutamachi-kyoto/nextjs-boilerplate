...
function generateReason(params: {
  ...
  const registeredHighRankPatterns = [
    ...
    `${offerName}は、${category}カテゴリの中でも現在の注目度が高く、ランキング上位に入りやすい条件がそろっています。${rewardText}の目安もあり、まず確認したい案件のひとつです。`,
    `${offerName}は、検索されやすいテーマとポイ活案件としての分かりやすさが重なっている案件です。上位候補として、AIが話題性と安定感を評価しました。`,
    `${offerName}は、今のポイ活ランキング内でも比較的目立つ案件です。報酬条件だけでなく、ユーザーが探しやすい案件かどうかも含めて評価しています。`,
    `${offerName}は、現在のトレンドと案件内容の結びつきが強く、上位表示に向いている案件です。申し込み前に条件を確認する価値が高い候補です。`,
    `${offerName}は、${focusPoint}の面で評価しやすく、登録済み案件の中でも優先度が高めです。話題性と報酬のバランスを見てAIが上位に配置しました。`,
    `${offerName}は、ポイ活初心者にも比較しやすい案件として評価しています。現在の注目度が高いため、ランキング上位の候補として採用しました。`,
    `${offerName}は、案件としての認知度と検索需要の両方が見られるため、今回のランキングで強めに評価しています。条件が合う人は早めに確認したい案件です。`,
  ];
  ...
  const registeredNormalPatterns = [
    ...
    `${offerName}は、${category}カテゴリの登録済み案件として、安定して比較しやすい候補です。話題性が極端に高い案件ではない場合でも、確認しておきたい案件として評価しています。`,
    `${offerName}は、ポイ活案件として内容を把握しやすく、ランキング内に入れる妥当性がある案件です。${focusPoint}を中心にAIが評価しました。`,
    `${offerName}は、検索需要との相性があり、候補から外すには惜しい案件です。報酬条件や対象サービスを確認しながら比較したい案件です。`,
    `${offerName}は、現在のランキング内で安定枠として評価できる案件です。派手さだけでなく、案件としての確認しやすさを重視しています。`,
    `${offerName}は、報酬ポイントの目安とカテゴリの分かりやすさから、比較対象に入れやすい案件です。申し込み条件を確認する前提で候補にしています。`,
    `${offerName}は、登録済み案件として一定の信頼感があり、ポイ活初心者にも検討しやすい案件です。今の検索動向を踏まえてランキングに反映しました。`,
    `${offerName}は、今すぐ大きく話題化している案件とは限りませんが、ポイ活案件としての相性は悪くありません。AIが比較候補として採用しています。`,
    `${offerName}は、案件内容とカテゴリの結びつきが分かりやすいため、ランキングに入れています。報酬だけでなく、探しやすさも含めて評価しました。`,
    `${offerName}は、他の案件と比較しながら検討しやすい登録済み案件です。現在の話題性と案件の安定感を見て、AIが候補に残しました。`,
    `${offerName}は、ポイ活案件として確認しやすい点が強みです。${rewardText}の目安がある場合は、条件とあわせて比較しておきたい案件です。`,
  ];
  ...
  const autoHighConfidencePatterns = [
    ...
    `${offerName}は、トレンドキーワードから見つかった候補の中でも、案件としての関連性が比較的高い案件です。${rewardText}の目安も確認できたため、自動発見枠の中では強めに評価しています。`,
    `${offerName}は、検索上の話題性とモッピー検索結果が結びついたことで検出された案件です。登録済みではないものの、条件確認する価値がある候補として採用しました。`,
    `${offerName}は、AIがトレンド検索の流れから抽出した案件です。報酬情報が比較的はっきりしているため、短期的に注目したい候補として扱っています。`,
    `${offerName}は、通常の登録案件とは別ルートで見つかったものの、${focusPoint}の面で評価できる案件です。トレンド性を重視してランキングに反映しました。`,
    `${offerName}は、最近の検索ニーズに反応して検出された案件です。報酬と案件名の関連が確認できたため、AI自動発見案件として上位寄りに評価しています。`,
    `${offerName}は、登録済み案件ではありませんが、検索トレンドとの一致度が高く、報酬情報も読み取れた案件です。今後の注目候補としてランキングに加えました。`,
    `${offerName}は、トレンド由来の案件の中でも、比較的判断材料がそろっている候補です。AIが話題性と報酬面を見て自動的に採用しました。`,
  ];
  ...
  const autoNormalPatterns = [
    ...
    `${offerName}は、トレンド由来で見つかった候補のため、登録済み案件よりも慎重な確認が必要です。ただ、現在の話題性を考えると、比較対象として残す価値があります。`,
    `${offerName}は、AIが検索動向から拾い上げた案件です。定番案件とは異なる可能性がありますが、今の関心に近い候補としてランキングに加えています。`,
    `${offerName}は、最近の検索キーワードとの関連から抽出された案件です。報酬条件や掲載状況が変わる可能性があるため、確認前提でチェックしたい候補です。`,
    `${offerName}は、通常の登録済み案件とは違い、トレンドから自動的に見つかった候補です。話題性を重視して、ランキング内に反映しています。`,
    `${offerName}は、検索上の動きから注目候補として検出されました。まだ安定案件とは言い切れませんが、今後伸びる可能性を見てAIが採用しています。`,
    `${offerName}は、案件としての確認には注意が必要ですが、トレンドとの関連性が見られます。短期的な候補として、ランキングに入れる価値があると判断しました。`,
    `${offerName}は、AIが自動的に発見した候補のひとつです。登録済み案件ほどの安定感はないものの、現在の話題性を反映する目的で採用しています。`,
  ];
